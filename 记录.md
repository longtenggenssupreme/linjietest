...
根据代码生曾数据库;
```
1.nuget包管理器--程序包管理器控制台
Install-Package Microsoft.EntityFrameworkCore
Install-Package Microsoft.EntityFrameworkCore.SqlServer  
Install-Package Microsoft.EntityFrameworkCore.Design

2. using ZhaoxiDbContext context = new ZhaoxiDbContext();
                #region CodeFirst 创建数据库 
                context.Database.EnsureDeleted();//删除数据库
                context.Database.EnsureCreated();//创建数据库 
                #endregion

                #region CodeFirst 数据迁移-创建数据库
                //add-migration addmigration001
                //update-database
```
代码生成数据库注意：
1. <font color=Red>先把原来dbfirst的项目去掉，否则代码生成数据数据迁移生成的时候会失败</font>
...
Scaffold-DbContext "Data Source=WIN-CSTBNBVVGSQ;Initial Catalog=ZhaoxiEduDataBase;User ID=sa;Password=sa123" Microsoft.EntityFrameworkCore.SqlServer -OutputDir Entity -Force -Context ZhaoxiDbContext -ContextDir /
命令参数：
...
```
生成数据库的脚本如下：
1. 先选择项目有见NuGet程序包管理，添加Microsoft.EntityFrameworkCore，Microsoft.EntityFrameworkCore.SqlServer，Microsoft.EntityFrameworkCore.Tools
2. 选择项目，之后选择工具栏中的工具 NuGet包管理器，程序包管理器控制台，输入Scaffold-DbContext "Data Source=WIN-CSTBNBVVGSQ;Initial Catalog=ZhaoxiEduDataBase;User ID=sa;Password=sa123" Microsoft.EntityFrameworkCore.SqlServer -OutputDir Entity -Force -Context ZhaoxiDbContext -ContextDir /
3. 选择项目设置为启动项，然后再程序包管理器控制台中选择默认项目，输入Scaffold-DbContext "Data Source=WIN-CSTBNBVVGSQ;Initial Catalog=ZhaoxiEduDataBase;User ID=sa;Password=sa123" Microsoft.EntityFrameworkCore.SqlServer -OutputDir Entity -Force -Context ZhaoxiDbContext -ContextDir /
4. 执行脚本，然后会生成对应的实体Entity和数据库上下文ZhaoxiDbContext

## 2021-1-7 WebAppNet5添加 IAuthorization权限验证子自定义权限
1. Startup ConfigureServices 中添加如下
```
 //注册自定义权限验证
            services.AddSingleton<IAuthorizationHandler, CustomAuthorizationHandler>();//添加自定义验证接口
...
## 2021-1-6 WebAppNet5添加 IActionFiltercookies session权限验证过滤器使用范围
1. Startup ConfigureServices 中添加 services.AddSession();
2. Startup Configure 中添加 app.UseSession();
3. 添加IActionFiltercookies session权限验证过滤器 CustomCookiesSessionActionFilterAttribute
```
/// <summary>
    /// cookies和session过滤器
    /// </summary>
...
4. 局部注册使用IActionFiltercookies session权限验证过滤器 CustomCookiesSessionActionFilterAttribute
```
...
## 2021-1-4 WebAppNet5添加 IExceptionFilter异常过滤器使用范围(捕捉哪些异常)
1. 控制器实例化异常 ---- 可以捕获异常
2. 异常发生在try-catch中 ---- 无法捕获异常
3. 视图中发生异常 ---- 无法捕获异常
4. Services层发生异常 ---- 可以捕获异常
5. 在Action中发生异常 ---- 可以捕获异常
6. 请求错误路径异常 ---- 可以捕获异常
```
 #region 请求错误路径异常
            ////请求错误路径异常
...
## 2020-12-31 WebAppNet5添加 Filter的定义 五种过滤器.NET5_IExceptionFilter异常过滤器
```
...
## 2020-12-29 WebAppNet5添加 Filter的定义 五种过滤器.NET5_IResourceFilter资源过滤器
```
/// <summary>
    /// 自定义资源过滤器，一般用作缓存
    /// </summary>
...
## 2020-12-22 WebAppNet5添加 Filter的定义 五种过滤器.NET5_Filter的生效范围和控制执行顺序
1. .NET5_Filter的生效范围和控制执行顺序
Global Filter过滤器执行前
Controller Filter过滤器执行前
Action Filter过滤器执行前
Action Filter过滤器执行后。。。。
Controller Filter过滤器执行后。。。。
Global Filter过滤器执行后。。。。
2. 改变执行顺序
```
1、继承ActionFilterAttribute，定义Filter的生效范围和控制执行顺序，使用IOrderedFilter种的Order属性设置顺序
...
```
3. 继承ActionFilterAttribute，定义Filter的生效范围和控制执行顺序，使用IOrderedFilter种的Order属性设置顺序
```
#region .NET5_Filter的生效范围和控制执行顺序==默认--修改，指定执行顺序 ActionFilterAttribute
    ///// <summary>
    ///// 全局的Filter过滤器
...
## 2020-12-22 WebAppNet5添加 Filter的定义 五种过滤器 IAuthorizationFilter,IResourceFilter,IActionFilter,IResultFilter,IExceptionFilter
1. 自定义特性
```
/// <summary>
...
2. 使用自定义特性
```
11、[ServiceFilter(typeof(CustomFilterAttribute))] 可以使用无参和有参构造函数 但 必须先注册服务
    [TypeFilter(typeof(CustomFilterAttribute))] 可以使用无参和有参构造函数，
    [CustomFilterAttribute] 只能使用无参，构造函数，不可以有参构造函数，
 //未使用构造函数依赖注入时，可以使用该特性[CustomFilter]标记,即自定义的filter过滤器CustomFilter，时必须要有无参构造函数
            //[TypeFilter(typeof(CustomFilterAttribute))]//可以使用无参构造函数也可以使用有参构造函数依赖注入其他需要的信息
            //[ServiceFilter(typeof(CustomFilterAttribute))]//可以使用无参构造函数也可以使用有参构造函数依赖注入其他需要的信息
...
```
3. .NET5_ServiceFilter_TypeFilter的原理_扩展定制IFilterFactory
```
